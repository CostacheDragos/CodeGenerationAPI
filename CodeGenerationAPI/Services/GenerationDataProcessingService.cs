...
{
    public class GenerationDataProcessingService : IGenerationDataProcessingService
    {
        ...
        public void PreProccessCodeGenerationNodes(List<ClassNodeModel> classNodes, List<PackageNodeModel> packageNodes)
        {
            ...
            foreach (var classNode in classNodes)
            {
                ...
                CheckMethodHeaderConflicts(classNode.ClassData);
        ...
        // Checks that in any given class, no 2 methods have the same name and the same header
        private void CheckMethodHeaderConflicts(ClassModel classModel)
        {
            if (classModel.Methods == null)
                return;

            // This dictionary will be used to determine if multiple methods have the same signature
            // Each element has the name of the methods as its key and the value
            // is being composed of a set of signatures associated with that method name
            Dictionary<string, HashSet<string>> methodsSignatures = new();
            ...
            foreach (var method in classModel.Methods)
            {
                ...
                if (method.Parameters != null)
                {
                    ...
                    {
                        string currentParameterSignature = parameter.Type.Name;

                        foreach (var pointer in parameter.Type.PointerList)
                            currentParameterSignature += pointer.IsConst ? "*const" : "*";

                        foreach (var dimension in parameter.Type.ArrayDimensions)
                            currentParameterSignature += "*";

                        signature += $"{currentParameterSignature} ";
                    }
                }

                // If the same method name exists, check if the same signature has been detected previously
                // if yes, throw an error
                if (methodsSignatures.ContainsKey(method.Name))
                {
                    if (methodsSignatures[method.Name].Contains(signature))
                        throw new GenerationException($"Method '{method.Name}' " +
                        $"from class '{classModel.Name}' has multiple definitions!");

                    methodsSignatures[method.Name].Add(signature);
                }
                else
                    methodsSignatures[method.Name] = new HashSet<string> { signature };
            }
        }
